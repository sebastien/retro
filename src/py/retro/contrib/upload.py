...
class Upload:
    ...
    def onCompleted(self, callback): return self.onStatus(
        self.IS_COMPLETED,   callback)

    def onFailed(self, callback): return self.onStatus(
        self.IS_FAILED,      callback)

    def onProgress(self, callback): return self.onStatus(
        self.IS_IN_PROGRESS, callback)
...
class Uploader:
    ...
    def upload(self, request, uploadID=None, chunksize=None):
        ...
        self.cleanup()
        upload = Upload(request=request, uid=request.param(
            "uid") if uploadID is None else uploadID)
        if chunksize:
