...
def shadow(password):
    ...
    p = b64encode(ensureBytes(pbkdf2_bin(password, salt,
                  COST_FACTOR, KEY_LENGTH, getattr(hashlib, HASH_FUNCTION))))
...
def crypt_decrypt(text, password, encoding="utf-8"):
    ...
    for position in range(len(text)):
        # Get next bias character from password
        bias = ord(password[position % len(password)])
