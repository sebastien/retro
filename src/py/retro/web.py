...
import traceback
import datetime
from retro.core import Request, Response, json, unjson, NOTHING, urllib_parse, ensureUnicode
from .compat import *
...
class WebRuntimeError(Exception):
    ...
    def __str__(self):
        res = self.message
        if self.code:
            res = "[{0}] {1}".format(self.code, res)
        if self.data:
            res = "{0}: {1}".format(res, self.data)
        return res
...
def cache(store, signature=None):
    ...
    def decorator(f):
        ...
        def wrapper(*args, **kwargs):
            key = cache_signature(
                f.__name__, args, kwargs) if signature is None else signature(f, args, kwargs)
            if store.enabled:
                result = None
                if store.has(key):
...
class Dispatcher:
        ...
        if isStart:
            ...
            if q != -1:
                ...
                if not (open_bracket_before >= 0 and close_bracket_before < open_bracket_before):
                    ...
                    if query:
                        params = query.strip()[1:-1].split(":")[0]
        while expression:
            ...
            if var_group != -1 and (var_group < opt_group or opt_group == -1):
                ...
                if end_group == -1:
                    raise DispatcherSyntaxError("Unclosed variable block")
                ...
                if not var_type.lower() in list(self.patterns.keys()):
                    ...
                    result += "(?P<%s>%s)" % (var_name,
                                              self.patterns[var_type.lower()][0])
            ...
            # Or we found an optional group
                ...
                if end_group == -1:
                    raise DispatcherSyntaxError("Unclosed optional group")
                # The optional group is like [...]
                _result, _convert, _ = self._parseExpression(
                    expression[opt_group+1:end_group], False)
                result += "(%s)?" % (_result)
    ...
    def decomposeExpression(expression):
        ...
        while offset < length:
            ...
            if var_group < opt_group:
                if var_group != offset:
                    chunks.append(('u', expression[offset:var_group]))
    ...
    def on(self, expression, prefix="", handlers={}, priority=0):
        ...
        if not prefix or prefix[-1] == "/":
            prefix = prefix[:-1]
        if not type(expression) in (tuple, list):
        ...
        for ex in expression:
            ...
            if LOG_DISPATCHER_ON:
                log("Dispatcher:", " ".join(["%4s" % (x)
                    for x in list(handlers.keys())]), ex)
            ...
            self._handlers.insert(
                0, (priority, regexp, params, converters, handlers))
    ...
    def info(self):
        ...
        for methods, url in self._routesInfo:
            res.append("Dispatcher: " +
                       " ".join(["%4s" % (x) for x in methods]) + " " + url)
        return "\n".join(res)
    ...
    def dispatch(self, environ, handlers=NOTHING, processor=(lambda r, h, v: h(r, **v)), request=None):
        ...
        # list)
        if handlers is NOTHING:
            handlers = self.match(environ)
        if request == None:
            ...
            if can_handle:
                try:
                    ...
                    for _ in self._onException:
                        _(e, self)
                    raise e if isinstance(
                        e, HandlerException) else HandlerException(e, request)
            else:
                handler = lambda request, **variables: Response(
                    "Not authorized", [], 401)
                return processor(request, handler, variables)
    ...
    def _processWSGI(self, request, handler, variables, start_response):
        ...
        if isinstance(response, Response):
            result = response.asWSGI(
                start_response, self.app.config("charset"))
            return result
        # TODO: Add an option to warn on None returned by handlers
            ...
            raise WebRuntimeError("Handler {0} for {1} should return a Response object, got {2}".format(
    ...
    def __call__(self, environ, start_response, request=None):
        ...
        def processor(r, h, v): return self._processWSGI(
            r, h, v, start_response)
...
class Application(Component):
    ...
    class EXPOSEWrapper:
        ...
        def __call__(self, request, **kwargs):
            ...
            for key, value in list(request.params().items()):
                if key and (key in self.functionArgs):
    ...
    def load(self, path, sync=True, mustExist=False):
        ...
        try:
            if sync:
                flags = flags | os.O_RSYNC
        except AttributeError as e:
            pass
        if not mustExist and not os.path.exists(path):
            return None
    ...
    def save(self, path, data, sync=True, append=False, mkdir=True):
        ...
        if not os.path.exists(parent) and mkdir:
            os.makedirs(parent)
    ...
    def register(self, *components):
        ...
        for component in components:
            ...
            if type(component) in (tuple, list):
                ...
                continue
            assert isinstance(
                component, Component), "retro.web: Cannot register object because it is not a Component instance: {0}".format(component)
    ...
    def component(self, nameOrClass):
        ...
        if type(nameOrClass) in (str, str):
            ...
            res = [c for c in self._components if c.__class__.__name__.lower()
                   == nameOrClass.lower()]
        else:
...
class Configuration:
    ...
    def _abspath(self, path):
        ...
        if type(path) in (tuple, list):
            return list(map(self._abspath, path))
