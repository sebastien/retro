...
class Request:
    ...
    def cookie(self, name, value=NOTHING, path="/"):
        ...
        if value is NOTHING:
            ...
            if c:
                return urllib_parse.unquote(c.value)
            else:
                return None
        else:
    ...
    def isLoaded(self):
        ...
        if self._bodyLoader:
            return self._bodyLoader.isComplete()
        else:
            return False
    ...
    def loadProgress(self, inBytes=False):
        ...
        if self._bodyLoader:
            return self._bodyLoader.progress(inBytes)
        else:
            return 0
    ...
    def _mergeHeaders(self, headersA, headersB=NOTHING):
        ...
        by default."""
        if headersB is NOTHING:
            headersB = self._responseHeaders
        if headersB:
...
class RequestBodyLoader:
    ...
    def __init__(self, request, complete=False):
        ...
        if complete:
            self.contentRead = self.contentLength
    ...
    def decode(self, dataFile=None):
        ...
        if not content_type:
            ...
            if len(charset) > 1:
                charset = charset[1].split(";")[0].strip()
