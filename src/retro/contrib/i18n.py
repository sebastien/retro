...
def T(text, lang=None):
    ...
    if lang in STRINGS[text]:
        ...
        return text
    logging.error(
        "Missing {0} translation for string: {1}".format(lang, repr(text)))
    return text
...
def guessLanguage(request):
    ...
    if type(request) in (str, unicode):
        ...
        if languages:
            ...
            languages = [_.split("-")[0].strip().lower()
                         for _ in languages.split(",")]
            if not LOCALES:
                return languages[0]
            else:
...
class Translations:
    ...
    def Add(cls, key, **languages):
        ...
        for lang, value in list(languages.items()):
            v[lang] = value
        return cls
