...
class LocalFiles(Component):
        ...
        if isinstance(resolved_path, list) or isinstance(resolved_path, tuple):
            ...
            if path.endswith(".gz"):
                ...
                if res.status >= 200 and res.status < 300:
                    res.setHeader("Content-Type", request.guessContentType(path)
    ...
    def _respondWithProcessor(self, request, processor, resolvedPath=None, multiPaths=None):
        if not multiPaths:
            try:
                content, content_type = processor(
                    self.getContent(resolvedPath), resolvedPath, request)
    ...
    def directoryAsHtml(self, path, localPath):
        ...
        if path and path not in ("/", "."):
            dirs.append(
                "<li class='previous dir'><span class='bullet'>&hellip;</span><a class='parent' href='%s/%s'>(parent)</a></li>" % (self.PREFIX, parent))
    ...
    def directoryAsList(self, path, localPath):
        ...
        files = []
        parent = os.path.dirname(path)
        local_files = list(os.path.join(parent, p)
                           for p in os.listdir(localPath))
        local_files.sort()
...
class LibraryServer(Component):
        ...
        if not self.cacheAggregates or not self._inCache(cache_path) or cache_path.find("+") == -1:
            ...
            for path in paths.split("+"):
                ...
                if not self._inCache(path):
                    ...
                    if data is None:
                        raise Exception(
                            "Processing path {0} returned None".format(path))
    ...
    def _processPath(self, path):
        ...
        if path.endswith(".sjs"):
            return self._processSJS(path)
        elif path.endswith(".js"):
            return self._processJS(path)
        elif path.endswith(".ccss"):
            return self._processCCSS(path)
        elif path.endswith(".pcss"):
            return self._processPCSS(path)
        elif path.endswith(".css"):
            return self._processCSS(path)
        elif path.endswith(".paml"):
            return self._processPAML(path)
        elif path.endswith(".xsl"):
            return self._processXSL(path)
        else:
            raise Exception("Format not supported: " + path)
    ...
    def _processPCSS(self, path):
        ...
        while (not data) and tries < 3:
            command = "%s %s" % (self.commands.get(
                "pythoniccss", "pythoniccss"), path)
    ...
    def _processSJS(self, path):
        ...
        while (not data) and tries < 3:
            command = "%s -cljs %s %s" % (
                self.commands["sugar"], "-L%s" % (self.library + "/sjs"), path)
            cmd = subprocess.Popen(command, shell=True, stdout=subprocess.PIPE)
            data = cmd.stdout.read()
            tries += 1
            cmd.wait()
        if data:
            if self.minify and jsmin:
