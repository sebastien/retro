...
class Robots:
    ...
    def ImportDB(cls, url="http://www.robotstxt.org/db/all.txt", encoding="latin-1"):
        ...
        for i, l in enumerate(f.readlines()):
            try:
                ...
                logging.error(
                    "retro.contrib.agents.Robots.ImportDB: Cannot decode line {0}:{1}".format(i, repr(l)))
                continue
            ...
            if l.startswith("robot-useragent:"):
                ...
                try:
                    ...
                    logging.error("retro.contrib.agents.Robots.ImportDB: User agent is not ascii at line {0}:{1}".format(
                        i, repr(robot_ua)))
                    continue
        return db
    ...
    def Detect(cls, request):
        if isinstance(request, retro.core.Request):
