...
def run(app=None, components=(), method=STANDALONE, name="retro",
    ...
    if method == FCGI:
        ...
        if fcgi_port and fcgi_address:
            server = FLUP_FCGIServer(
                stack, bindAddress=(fcgi_address, fcgi_port))
        elif fcgi_address:
    ...
    #
        ...
        def application(environ, startResponse):
            ...
            if "retro.app" not in environ:
                environ["retro.app"] = stack.app
        ...
        if method == "GEVENT":
            ...
            wsgi.WSGIServer((host, port), application,
                            spawn=None).serve_forever()
        elif method == BJOERN:
            try:
                import bjoern
            except ImportError:
            ...
            rocket.Rocket((host, int(port)), "wsgi", {
                          "wsgi_app": application}).start()
        elif method == WSGI:
    ...
    #
        ...
        if method == STANDALONE and not asynchronous:
            ...
            try:
                ...
                socket = server.socket.getsockname()
                print("Retro embedded server listening on %s:%s" %
                      (socket[0], socket[1]))
            except Exception as e:
                logging.error("Retro: Cannot bind to {0}:{1}, error: {2}".format(
                    server_address[0], server_address[1], e))
                return -1
