...
def on( priority=0, **methods ):
	...
	Railways. This decorator allows to wrap an existing method and indicate that
...
class Dispatcher:
	...
	Selector module, only more tailored to the needs of Railways (that is
	...
	subclassing it was a license problem: I wanted Railways to be BSD, while
	Selector was GPLed."""
	...
	def app( self ):
		"""Returns a reference to the Railways application bound to this
		dispatcher."""
		return self._app
	...
	def __call__(self, environ, start_response, request=None):
		...
		def handle( handler, variables, start_response):
			...
			request.environ("railways.start_response", start_response)
...
class Component:
	...
	the decorators provided by Railways and register the component into the
	application."""
	...
	def introspect( component ):
		...
		for each slot that was decorated by Railways decorators."""
		res = []
...
class Application(Component):
	...
	class TemplateWrapper:
		...
		def __call__( self, request, **kwargs ):
			...
			#for key, value in request.environ("railways.variables").items(): context[key] = value
	...
	def register( self, *components ):
		...
		introspected to see if there are decorated with Railways decorators. If
...
class Configuration:
	...
	Railways applications. The supported properties are the following:
